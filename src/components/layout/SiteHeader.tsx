...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-charcoal/5 bg-ivory/85 backdrop-blur-xl">
      <div className="luxe-container flex h-16 items-center justify-between gap-4">
        ...
        <nav className="hidden items-center gap-7 md:flex">
          {links.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className="text-sm font-medium text-charcoal/70 transition hover:text-gold-dark"
            >
              {l.label}
            </Link>
          ))}
        ...
        <nav className="border-t border-charcoal/5 bg-ivory px-5 py-3 md:hidden">
          {links.map((l) => (
            <Link
              ...
              className="block py-2 text-sm font-medium text-charcoal/80"
            >
              {l.label}
            </Link>
          ))}
