...
export function EmployeeRequestView({
  request,
  whatsappNumber,
}: {
  request: Data;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
          ...
          <div className="luxe-card p-5">
            ...
            {request.contactMeInstead && (
              <p className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800">
                {pick(t.details.contactMe)}
              </p>
            )}
            <div className="mt-4 flex flex-wrap gap-2">
              <a href={`tel:${request.customer.phone}`} className="btn-outline px-3 py-2 text-xs">
                {pick(t.admin.contactClient)}
              </a>
              <a
              ...
              <button onClick={copyWa} className="btn-outline px-3 py-2 text-xs">
        ...
        <div className="space-y-5">
          ...
          <div className="luxe-card p-4">
            <h4 className="mb-3 text-sm font-semibold text-charcoal">
              {pick(t.admin.internalNotes)}
            </h4>
            <TextArea
              value={note}
              onChange={(e) => setNote(e.target.value)}
              placeholder={pick(t.admin.addNote)}
            />
            <div className="mt-2 flex gap-2">
              <button
                ...
                onClick={() =>
                  run(async () => {
                    await adminAddNote(request.id, note);
                    setNote("");
                  })
                }
              ...
              <button
                ...
                onClick={() =>
                  run(async () => {
                    await employeeEscalate(request.id, note);
                    setNote("");
                  })
                }
            ...
            <ul className="mt-3 space-y-2">
              {request.internalNotes.map((n) => (
                <li key={n.id} className="rounded-lg bg-ivory-warm p-3 text-sm text-charcoal/80">
                  {n.body}
                </li>
              ))}
            </ul>
